  ...
  return (
    <main className="px-40 py-20 pr-0 flex justify-between items-center h-screen bg-grad text-white">
      <div className="text flex flex-col justify-between items-start h-full max-w-lg">
        ...
        <button onClick={() => router.back()} className="px-12 py-5 font-bold text-5xl bg-white bg-opacity-5 rounded-full font-mainSlim flex gap-2 items-center">
          <span className="text-pur">&lt;</span>go back
          <span className="text-pur">/&gt;</span>
          <Image src="/we think/star.png" width={27} height={27} />
        </button>
        <div className="underline bg-line h-6 fixed bottom-0 left-0 w-screen"></div>
      </div>
      <div className="image relative flex justify-end items-center">
        <Link href={`/${img}`}>
          <Image
            ...
            width={614}
            height={392}
            className="relative z-10 rounded-l-3xl my-transition"
          />
        </Link>
        <Image
